...
void do_work(void);
void shutdown(void);
void kick_dog(void);               /* Creating kick_dog(void) */
void set_timer_count(int count);   /* Creating set_timer_count(int count) */
...
// Function kick_dog() which calls the set_timer_count() function
void kick_dog(void) {
	set_timer_count(0); /* reset the count back to 0 */
}
...
void do_work(void)
{
   ...
   kick_dog(); /* implementing kickdog() at the end of do_work() function */
}
